...
public interface CategoryReportMapper extends Mapper<CategoryReport> {
    ...
    List<CategoryReport> categoryReport(@Param("date") LocalDate localDate);

    @Select("SELECT  r.category_id1 categoryId1, c.`NAME` categoryName,SUM(num) num,SUM(money) money FROM tb_category_report r,v_category1 c " +
            "WHERE r.category_id1 = c.id AND r.count_date >= #{date1} AND r.count_date <= #{date2} " +
